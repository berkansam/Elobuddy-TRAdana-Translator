...
namespace OKTRAIO.Utility
{
    ...
    public class RandomUlt : UtilityAddon
    {
        ...
        {
            Menu.AddGroupLabel("OKTR AIO - Rastgele Ulti for " + Player.Instance.ChampionName,
                "randomult.grouplabel.utilitymenu");
            Menu.AddCheckBox("randomult.use", "Kullan  Rastgele Ulti");
            Menu.Add("randomult.advanced", new CheckBox("Gelişmiş Ayar Menüsünü Göster", false)).OnValueChange +=
                Value.AdvancedModeChanged;
            Menu.AddSeparator();
            Menu.AddSlider("randomult.range", "Düşman Menzildeyken Ulti Kullanma", 1000, 0, 2000, true);
            Menu.AddSlider("randomult.delay", "Ultiden önce gecikme", 600, 0, 3000, true);
            Menu.AddSlider("randomult.hitchance", "Rastgele Ulti Tutma Oranı (1 - Daha Fazla Ulti, 5 - Daha az Ulti)", 3, 1, 5,
                true);
