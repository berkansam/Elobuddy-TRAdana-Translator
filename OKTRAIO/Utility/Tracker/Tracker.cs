...
namespace OKTRAIO.Utility.Tracker
{
    public class Tracker : UtilityAddon
    {
        ...
        {
            return new UtilityInfo(this, "Takipçi", "tracker", "coman3");
        }

        protected override void InitializeMenu()
        {
            Menu.AddGroupLabel("OKTR AIO - Takipçisi", "tracker.grouplabel.utilitymenu");
            Menu.AddLabel("Developed By: Coman3");
            Menu.AddLabel("Ceviri tradana");
            Menu.AddCheckBox("tracker.enable", "Aktif", false);
            Menu.AddLabel("Takipçi Henüz Tamamlanmadı Ama Çalışıyor");
            Menu.AddCheckBox("tracker.show.spells.summoner", "Sihirdar Büyülerini Göster");
            Menu.AddCheckBox("tracker.show.spells.normal", "Normal Büyüleri Göster");
            Menu.AddCheckBox("tracker.show.player.me", "Beni Takip Et");
            Menu.AddCheckBox("tracker.show.player.allies", "Dostları Takip Et");
            Menu.AddCheckBox("tracker.show.player.enemies", "Düşmanları Takip Et");
            Menu.Add("tracker.reload", new CheckBox("Takipçiyi Tekrar Yükle", false)).OnValueChange += Reload;

            Menu.AddSeparator();
            Menu.Add("tracker.advanced", new CheckBox("Gelişmiş Menü", false)).OnValueChange += Value.AdvancedModeChanged;
            Menu.AddCheckBox("tracker.show.onlyhpbarrendered", "Sadece Gizlenmiş Düşmanları Takip Et", false, true);
            Menu.AddLabel("This is currently buggy as the core does not fully support this option");
            Menu.AddLabel("Çevirmen Notu:Yukarıyı Umursamayın");
