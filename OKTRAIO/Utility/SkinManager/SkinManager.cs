...
namespace OKTRAIO.Utility.SkinManager
{
    public class SkinManagement : UtilityAddon
    {
        ...
        {
            Menu.AddGroupLabel("OKTR AIO - SkinDeğiştirici for " + Player.Instance.ChampionName,
                "skinmanager.grouplabel.utilitymenu");
            Menu.AddLabel("PSA: Model Değiştirirken Çökmeye neden olabilir" + Environment.NewLine +
                                 "Bu Addon Değiştirdiğin Skini Kayıt Etmez");
            Menu.AddSeparator();
            Menu.Add("skinmanager.models", new Slider("Model - ", 0, 0, 0)).OnValueChange +=
                SkinManager_OnModelSliderChange;
            Menu.Add("skinmanager.skins", new Slider("Skin - Klasik", 0, 0, 0)).OnValueChange +=
                SkinManager_OnSkinSliderChange;
            Menu.Add("skinmanager.resetModel", new CheckBox("Sıfırla Model", false)).OnValueChange +=
                SkinManager_OnResetModel;
            Menu.Add("skinmanager.resetSkin", new CheckBox("Sıfırla Skin", false)).OnValueChange +=
