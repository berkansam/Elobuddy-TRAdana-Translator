...
namespace OKTRAIO.Champions
{
    internal class Jhin : AIOChampion
    {
        ...
        public override void Init()
        {
            ...
            MainMenu.Combo.AddGroupLabel("Combo Ayarları", "combo.settings.label", true);
            MainMenu.Combo.AddLabel("Q Ayarları", 25, "combo.q.settings.label", true);
            var qMode = new List<string> { "Use on Enemy", "Try 100% Damage" };
            MainMenu.Combo.AddComboBox("combo.q.mode", "Komboda Q modu", qMode, 0, true);

            //LaneClear
            MainMenu.LaneKeys(true, true, true,true,false,false,false,false);
            MainMenu.Lane.AddGroupLabel("Lane Clear Ayarları", "lane.settings.label", true);
            MainMenu.Lane.AddLabel("Q Ayarları", 25,"lane.q.settings.label",true);
            MainMenu.Lane.AddSlider("lane.q.minions", "Eğer şu kadar minyon ölecekse Q kullan:", 1, 1, 3, true);
            #endregion

            Value.Init();
            Notifications.Show(new SimpleNotification("One Key to Report","Jhin Yuklendi!- ceviri Tradana"));
        }
