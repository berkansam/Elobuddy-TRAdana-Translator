...
namespace ToasterBuddy
{
    public static class ToasterBuddy
    {
        ...
        private static string _toasterText = "Paketler Yüklenirken Bekleyin.";
        private static int _timeLimit = 300;
        ...
        private static void Game_OnTick(EventArgs args)
        {
            if (ToasterGamePacketIsReady)
            {
                ...
                {
                    _toasterText = "Paketler Gönderildi, Oyun Başlayacak :) İyi Eğlenceler.";
                }
                else
                {
                    _toasterText = "Paket Bulundu , Oyun başladıktan sonra sen yapmalısın (Kullan esc veya space).\nTime Left: " + Math.Truncate(_timeLimit - (Game.Time - _startTime)) + "s\n Eğer silme süresi yapacaksan, Bas senin Yukarı/Aşağı tuşlarına klavyeden.";
                }
            }
        }
