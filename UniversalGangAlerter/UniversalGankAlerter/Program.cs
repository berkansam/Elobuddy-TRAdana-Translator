...
namespace UniversalGankAlerter
{
    class Program
    {
        ...
        private void Game_OnGameLoad(EventArgs args)
        {
            ...
            _menu = MainMenu.AddMenu("Gank Uyarıcı", "universalgankalerter");
            _sliderRadius = new Slider("Trigger range", 3000, 500, 5000);
            _sliderRadius.OnValueChange += SliderRadiusValueChanged;
            _sliderCooldown = new Slider("Trigger Bekleme Süresi (saniye)", 10, 0, 60);
            _sliderLineDuration = new Slider("Çizgi Süresi (saniye)", 10, 0, 20);
            _enemyJunglerOnly = new CheckBox("Düşman Jungle büyüsü için uyar (Çarp)", false);
            _allyJunglerOnly = new CheckBox("Dost Jungle büyüsü için uyar (Çarp)", true);
            _showChampionNames = new CheckBox("Şampiyon isimlerini göster", true);
            _drawMinimapLines = new CheckBox("Küçük Haritada Son yerleri göster", false);
            _dangerPing = new CheckBox("Tehlike Pingi Kullan", false);
            _enemies = _menu.AddSubMenu("Düşmanlar", "enemies");
            _enemies.Add<CheckBox>("enemyjungleronly", _enemyJunglerOnly);

            _allies = _menu.AddSubMenu("Dostlar", "allies");
            ...
            //_menu.AddToMainMenu();
            Print("Loaded! Tradana çevirisi kullandığınız için teşekkürler");
        }
